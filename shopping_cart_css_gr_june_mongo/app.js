...
app.post('/products/:id/purchaseitems', async(req, res) => {
    ...
    if(!purchaseItemByDate) {   
        ...
        const updatePurchaseOrder = async (req, res) => { 
            // Req.body
            let updatePurchase = purchaseItemById
            console.log("This is a updateProject")
            console.log(updatePurchase)
            // Set empty OBJ
            const set = {'updatedAt': new Date() + 2 * 1000};
              // Loop the fields to update
              for(const field in updatePurchase) {
                console.log("This is field in updatePurchase loop")
                console.log(`This is ${field} in ${updatePurchase} loop`)
             ...
             const selectedPurchaseItem = new Purchaseitem.findById(id)
             const purchaseitemToUpdate = await new Purchaseitem.updateOne({
                // We are trying to update the purchaseitem._id here
                "purchaseitem._id": selectedPurchaseItem._id
            },
            { $set: set },
            {new:true})
            console.log("This is field in purchaseitemToUpdate")
            console.log(purchaseitemToUpdate)
            return purchaseitemToUpdate
        }
        let updateUpdatedAtDate = updatePurchaseOrder(purchaseItemById)
        console.log("This is field in updateUpdatedAtDate")
        console.log(`This is the ${updateUpdatedAtDate}`)
